...
def main():
    ...
    cv2.setMouseCallback("IR Runway Clearance", mouse_callback)
    ...
    while True:
        ...
        # --- Optionally invert for "thermal" effect ---
        fake_ir = cv2.bitwise_not(gray)

        # Convert single-channel back to 3-channel so YOLO works
        fake_ir_3ch = cv2.cvtColor(fake_ir, cv2.COLOR_GRAY2BGR)

        # --- YOLO detection on fake IR frame ---
        try:
